            ...
            {SORT_OPTIONS.map((val) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setSort(val)}
                  className={`rounded-lg border-[3px] px-4 py-2.5 text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900 focus-visible:ring-offset-2 ${
                    active
                      ? "border-zinc-900 bg-[#fff4da] text-zinc-900"
...
function PromptCard({ entry }: { entry: PromptEntry }) {
  ...
  const githubHref = `https://github.com/${encodeURIComponent(entry.owner)}/${encodeURIComponent(entry.repo)}`;
  const truncated =
    entry.prompt.length > 300
      ? entry.prompt.slice(0, 300).trimEnd() + "…"
      : entry.prompt;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[200px] h-full flex-col gap-4 rounded-xl border-[3px] border-zinc-900 bg-white p-5 transition-transform group-hover:-translate-x-0.5 group-hover:-translate-y-0.5">
        ...
        <div className="mt-auto flex flex-col gap-3 border-t border-zinc-100 pt-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border-[3px] border-zinc-900 bg-white px-3 py-1.5 text-xs font-semibold text-zinc-900 transition-colors hover:bg-zinc-50"
            >
              <svg
